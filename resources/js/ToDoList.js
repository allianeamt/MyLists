...
function ToDoList () {
	...
	async function openModal (id) {
		let result = await fetch("http://localhost:8000/api/getItem/"+id)
	...
	{
		await fetch("http://localhost:8000/api/updateCheck/"+id,{
			method:'POST'
		})
		update()
	}
	...
	return (
		<div>
			<Modal children={children} data={data} idlist={idlist} check={checkModal} setCheck={setCheckModal} setList={setList} imagePath={imagePath} setImagePath={setImagePath} nameItem={nameItem} 
					...
					<div>
					{
						...
						list.map((item)=>
							item.done ? 
								null
								:
								<div className="todo-row" key={item.id}>{item.name}
									<div className="icons">
										{
											item.done ?
						...
						list.map((item)=>
							item.done ? 
								<div className="todo-row-cut" key={item.id}>{item.name}
									<div className="icons">
										{
											item.done ?
												<MdClose className='check-icon' onClick={()=>checkItem(item.id,item.done)} />
